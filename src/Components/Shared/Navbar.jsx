...
function Navbar ({DashboardLogo, indexPage}){
    ...
    return (
                ...
                <div className={`nav-links bg-[#0C161D] border  text-white text-xl flex justify-between items-center w-[75%] px-28 rounded-full max-lg:px-15 z-2 max-md:flex-col max-md:px-4 max-md:pt-4 max-md:pb-8 max-md:absolute max-md:top-8 max-md:w-auto max-md:top-0 max-md:relative max-md:z-4  max-md:rounded-[10px]  max-md:justify-start max-md:gap-4 ${!isNavOpen ? "max-md:right-[-500px]" : "max-md:right-0"}`}>
                ...
                {isIndexPage && 
                <div className="flex items-center justify-center bg-black text-white text-xl  rounded-full z-3 border py-4 px-8">
                    <Link to="/login">
                        Log in
                    </Link>
                </div>
